...
public:
    ...
    void on_read(std::list<Connection>::iterator con_handle, boost::system::error_code const &err, size_t bytes_transfered) //метод чтения сообщений
    {
        if (bytes_transfered > 0)//если количесво прочитанных байт больше 0
        {
            ...
            std::cout << line << std::endl;
                if (line =="username\r"||line == "hostname\r" || line == "serverdatetime\r" || line.substr(0, 9) == "mkfile ~/"
                ||line.substr(0, 8) == "mkdir ~/" || line.substr(0, 5) == "ls ~/" || line == "quit\r"
                ||line.substr(0, 8) == "rmdir ~/" || line.substr(0, 9) == "rmfile ~/"
                || line.substr(0, 11) == "readfile ~/"){
    ...
    void handle_command(std::list<Connection>::iterator con_handle , const std::string& cmd) //обработчик специальных команд
    {
        ...
        if (cmd == "username") { // если строка == username
        ...
        } else if (cmd.substr(0, 5) == "ls ~/") { //проверяем первые 3 символа строки, если они == ls , то обрабатываем
            boost::process::ipstream out;//создание потока для чтения вывода команды
            std::string command = "ls " + cmd.substr(5);
            boost::process::system(command, boost::process::std_out > out);// выполнение команды и запись ответа в out
            ...
            if (tokens.size() >= 2) { //проверка на наличие имени файла
                std::string path = tokens[1].substr(2); //выносим в переменную имя файла
